...
export async function recordCostPg(entry: CostEntry): Promise<void> {
  ...
  await pool.query(
    `INSERT INTO cost_entries (timestamp, agent, model, input_tokens, output_tokens, cost_usd, duration_ms)
     VALUES ($1, $2, $3, $4, $5, $6, $7)`,
    [entry.timestamp, entry.agent, entry.model, entry.inputTokens, entry.outputTokens, entry.costUsd, entry.durationMs],
  );
}
